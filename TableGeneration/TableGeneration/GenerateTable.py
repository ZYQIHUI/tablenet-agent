...
class GenerateTable:
    def __init__(
        ...
        if self.brower == "chrome" and self.chrome_driver_path:
            from selenium.webdriver.chrome.service import Service
            service = Service(str(Path(self.chrome_driver_path).resolve()))
            self.driver = Brower(service=service, options=opts)
        else:
    ...
    def render_table(self, html_content, structure, id_count, border):
        im, contens = self.html_to_img(html_content, id_count)
        im, contens = self.clip_white(im, contens)
        return im, html_content, structure, contens, border
    ...
    def html_to_img(self, html_content, id_count):
        ...
        self.driver.get("data:text/html;charset=utf-8," + quote(html_content))
